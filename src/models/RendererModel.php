...
class RendererModel
{
    ...
    {
        if (!$lines) {
            throw new InvalidException("Lines parameter must be set.");
        }
    ...
    private function checkNumberPositive($num, $field)
    {
        ...
        if ($digits <= 0) {
            throw new InvalidException("$field must be a positive number.");
        }
        return $digits;
    }
    ...
    private function checkNumberNonNegative($num, $field)
    {
        $digits = intval(preg_replace("/[^0-9\-]/", "", $num));
        if ($digits < 0) {
            throw new InvalidException("$field must be a non-negative number.");
        }
        return $digits;
    }
